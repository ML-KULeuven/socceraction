...
def test_predict(sb_worldcup_data):
    ...
    features = pd.concat([model.compute_features(game, atomic_actions[game.game_id]) for game in games.iloc[:-1].itertuples()])
    ...
    labels = pd.concat([model.compute_labels(game, atomic_actions[game.game_id]) for game in games.iloc[:-1].itertuples()])
