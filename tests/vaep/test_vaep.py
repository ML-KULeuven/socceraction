...
def test_predict(sb_worldcup_data):
    ...
    features = pd.concat([model.compute_features(game, sb_worldcup_data[f'actions/game_{game.game_id}']) for game in games.iloc[:-1].itertuples()])
    ...
    labels = pd.concat([model.compute_labels(game, sb_worldcup_data[f'actions/game_{game.game_id}']) for game in games.iloc[:-1].itertuples()])
    ...
    expected_rating_columns = set(list(actions.columns) + ['offensive_value', 'defensive_value', 'vaep_value'])
