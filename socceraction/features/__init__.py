...
gamestates
   Generators which calculate a set of features based on a SPADL action and
   ...
   :math:`a_{i-1}` dataframe. :math:`a_0` is the action for which the features
   are calculated.

actions
   Generators which calculate a set of features based on the current and
   all preceding actions. The input is a :class:`pandas.DataFrame` of actions
   in SPADL format and a boolean mask to select the actions for which features
   should be computed.

events
   Generators which calculate a set of features based on the original
   ...
   the actions for which features should be computed.
...
As the "gamestates" and "actions" generators compute features from
...
from .spadl.possession import player_possession_time, team
from .spadl.result import actiontype_result_onehot, result, result_onehot
from .spadl.sequence import space_delta, speed, time_delta
