...
class PublicWyscoutLoader(EventDataLoader):
    ...
    def _create_match_index(self) -> DataFrame:
        ...
        str_ids = ["match_id", "competition_id", "season_id"]
        for str_id in str_ids:
            df_matches[str_id] = df_matches[str_id].astype("Int64").astype(str)
        return pd.merge(
    ...
    def competitions(self) -> DataFrame[WyscoutCompetitionSchema]:
        ...
        df_competitions["country_name"] = df_competitions.apply(
            lambda x: x.area["name"] if x.area["name"] != "" else "International",
            axis=1,
        )
        ...
        str_ids = ["competition_id", "season_id"]
        for str_id in str_ids:
            df_competitions[str_id] = df_competitions[str_id].astype(str)
    ...
    def games(self, competition_id: str, season_id: str) -> DataFrame[WyscoutGameSchema]:
        ...
        for str_id in str_ids:
            df_matches[str_id] = df_matches[str_id].astype(str)
...
class WyscoutLoader(EventDataLoader):
    ...
    def _get_file_or_url(
        ...
        glob_pattern = self.feeds[feed].format(
            competition_id=competition_id_glob,
            season_id=season_id_glob,
            game_id=game_id_glob,
        )
        if "*" in glob_pattern:
...
def convert_seasons(seasons: pd.DataFrame) -> pd.DataFrame:
    ...
    seasons["competition_id"] = seasons["competition_id"].astype(str)
    seasons["season_id"] = seasons["season_id"].astype(str)
    return seasons
...
def convert_games(matches: pd.DataFrame) -> pd.DataFrame:
    ...
    str_ids = ["game_id", "competition_id", "season_id"]
    for str_id in str_ids:
        games[str_id] = games[str_id].astype(str)
    return games


def get_team_id(teamsData: Dict[str, Any], side: str) -> str:
...
def convert_players(players: pd.DataFrame) -> pd.DataFrame:
    players["wyId"] = players["wyId"].astype(str)
    playermapping = {
        "wyId": "player_id",
        "shortName": "nickname",
        "firstName": "firstname",
...
def convert_teams(teams: pd.DataFrame) -> pd.DataFrame:
    teams["wyId"] = teams["wyId"].astype(str)
    teammapping = {
        "wyId": "team_id",
...
def convert_events(raw_events: pd.DataFrame) -> pd.DataFrame:
    ...
    events["subtype_id"] = (
        pd.to_numeric(
            events["sub_event_id"] if "sub_event_id" in events.columns else None,
            errors="coerce",
        )
        .fillna(0)
        .astype(int)
    ...
    events = events.rename(columns=eventmapping)[cols]
    str_ids = ["game_id", "player_id", "team_id"]
    for str_id in str_ids:
        events[str_id] = events[str_id].astype(str)
    return events
...
def get_minutes_played(teamsData: pd.DataFrame, events: pd.DataFrame) -> pd.DataFrame:
    ...
    df = pd.DataFrame(playergames.values())
