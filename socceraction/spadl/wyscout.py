...
def get_players(root):
    return [player["player"] for team in root["players"].values()
           for player in team if player.get("player")]
...
def get_player_games(match,events):
    ...
    for team_id,teamData in teamsData.items():
        ...
        pg =  {player["playerId"] : {"game_id" : game_id,
                                     "team_id" : team_id,
...
def augment_events(events_df):
    ...
    events_df["type_id"] = events_df["eventId"] if "eventId" in events_df.columns else events_df.eventName
    events_df["subtype_id"] = events_df["subEventId"] if "subEventId" in events_df.columns else events_df.subEventName
    events_df["period_id"] = events_df.matchPeriod.apply(lambda x: int(x.replace("H","")))
...
def make_new_positions(events_df):
    new_positions = events_df[['id','positions']].apply(lambda x: make_position_vars(x[0],x[1]),axis = 1)
...
def create_shot_coordinates(df_events):
    ...
    """
    goal_center_idx = df_events['position_goal_low_center'] | df_events['position_goal_mid_center'] | df_events['position_goal_high_center']
    df_events.loc[goal_center_idx, 'end_x'] = 100.0
    df_events.loc[goal_center_idx, 'end_y'] = 50.0

    goal_right_idx = df_events['position_goal_low_right'] | df_events['position_goal_mid_right'] | df_events['position_goal_high_right']
    df_events.loc[goal_right_idx, 'end_x'] = 100.0
    df_events.loc[goal_right_idx, 'end_y'] = 55.0

    goal_left_idx = df_events['position_goal_mid_left'] | df_events['position_goal_low_left'] | df_events['position_goal_high_left']
    ...
    out_right_idx = df_events['position_out_low_right'] | df_events['position_out_mid_right'] | df_events['position_out_high_right']
    df_events.loc[out_right_idx, 'end_x'] = 100.0
    df_events.loc[out_right_idx, 'end_y'] = 60.0

    out_left_idx = df_events['position_out_mid_left'] | df_events['position_out_low_left'] | df_events['position_out_high_left']
    df_events.loc[out_left_idx, 'end_x'] = 100.0
    df_events.loc[out_left_idx, 'end_y'] = 40.0

    post_left_idx = df_events['position_post_mid_left'] | df_events['position_post_low_left'] | df_events['position_post_high_left']
    df_events.loc[post_left_idx, 'end_x'] = 100.0
    df_events.loc[post_left_idx, 'end_y'] = 55.38

    post_right_idx = df_events['position_post_low_right']| df_events['position_post_mid_right'] | df_events['position_post_high_right']
...
def insert_interception_passes(df_events):
    ...
    df_events_interceptions = df_events[df_events['interception'] & (df_events['type_id'] == 8)].copy()
...
def create_df_actions(df_events):
    ...
    df_actions = df_events[['game_id', 'period_id', 'time_seconds', 'team_id',
                            'player_id', 'start_x', 'start_y', 'end_x', 'end_y']].copy()
...
def determine_result_id(event):
    ...
    if event['offside'] == 1:
        ...
        return 0
    elif event['interception'] or event['clearance'] or event['subtype_id'] == 71:  # interception or clearance always success
        return 1
...
def fix_actions(df_actions):
    ...
    df_actions['start_y'] = (100 - df_actions['start_y']) * spadl_width / 100  # y is from top to bottom in Wyscout
    df_actions['end_x'] = df_actions['end_x'] * spadl_length / 100
    df_actions['end_y'] = (100 - df_actions['end_y']) * spadl_width / 100 # y is from top to bottom in Wyscout
...
def remove_keeper_goal_actions(df_actions):
    ...
    shot_goals = (prev_actions.type_id == actiontypes.index('shot')) & (prev_actions.result_id == 1)
    penalty_goals = (prev_actions.type_id == actiontypes.index('shot_penalty')) & (prev_actions.result_id == 1)
    freekick_goals = (prev_actions.type_id == actiontypes.index('shot_freekick')) & (prev_actions.result_id == 1)
